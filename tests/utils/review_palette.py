...
# adapted from:
# author: https://stackoverflow.com/users/5519975/tristan
# source: https://stackoverflow.com/questions/39194402/invert-values-of-colour-in-css
# function takes in a hex color and outputs it inverted

#updated [1:] to [0:]
def invert(color_to_convert): 
    table = str.maketrans('0123456789abcdef', 'fedcba9876543210')
    return '#' + color_to_convert[0:].lower().translate(table).upper()
    ...
    for p in palettes:
        ...
        for rule in sheet:
            if isinstance(rule, cssutils.css.csscomment.CSSComment):
                continue
            # this part assumes that each line ends with a semicolon
            style_text = rule.style.cssText.replace("\n", "")
            for i in style_text.split(";"):
                pair = i.split(": ")
                label = pair[0].replace("--", "")
                value = pair[1].replace("#", "")
                # we might be able to compute this on the fly 
                # would need to look up the variable?  or possibly adjust the library to resolve the variables

                # https://pythonhosted.org/cssutils/docs/serialize.html#cssutils.serialize.Preferences
                # resolveVariables = True
                # or maybe there is a way to add color definitions
                # cssutils.css.ColorValue.COLORS
