...
class TestTile():
    ...
    def xtest_empty_tile(self, tile_macro, empty_tile):
        rendered_tile = tile_macro.module.make_tile(empty_tile)
        assert rendered_tile.strip() == ""

    def xtest_minimal_link_tile(self, tile_macro, minimal_link_tile):
    ...
    def xtest_minimal_image_tile(self, tile_macro, minimal_image_tile):
    ...
    def xtest_minimal_linked_img_tile(self, tile_macro, minimal_linked_image_tile):
    ...
    def xtest_id_and_class_added_to_tile(self, tile_macro):
    ...
    def xtest_text_included_in_link_only_tile(self, tile_macro):
        tile = Tile(text="link_display_text", link_href=defaults.GITHUB_LINK_HREF)
        rendered_tile = tile_macro.module.make_tile(tile)
        assert ">link_display_text</a>" in rendered_tile
        assert_valid_html(rendered_tile)

    def xtest_text_used_for_image_alt_when_image_included(self, tile_macro, valid_linked_image_tile):
    ...
    def xtest_that_tile_renders_with_integer_inputs(self, tile_macro, all_integer_tile):
        try:
            tile_macro.module.make_tile(all_integer_tile)
        except Exception as ex:
            pytest.fail(f"Got exception during render: {ex})")

    def xtest_caption_when_run_through_markup_filter(self, tile_macro, valid_linked_image_tile):
    ...
    def xtest_caption_when_markup_filter_not_used(self, tile_macro, valid_linked_image_tile):
        valid_linked_image_tile.caption = "myCaption"
        rendered_tile = tile_macro.module.make_tile(valid_linked_image_tile, False)
